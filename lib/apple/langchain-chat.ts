...
export class LangChainGeminiChat {
  ...
  async chat(params: {
    ...
    const { sessionId, userId, message, imageUrls = [], systemPrompt } = params;
    ...
    // Load historical messages from KV
    const historicalMessages = await this.loadHistoryAsMessages(sessionId);
    console.log(
      `[LangChain Chat] Loaded ${historicalMessages.length} historical messages`
    );
