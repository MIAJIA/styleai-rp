...
Your response should include BOTH text description AND generated images.`;
        ...
        const response = await fetch('https://styleai.h.zzn.im/api/apple/aichat', {
            headers: {
        ...
        // 正确的方式：直接从 response 对象解析 JSON
        const responseData = await response.json();
        console.log(`[Chat API] Response received, success: ${responseData.success}`);
        
        // aichat 返回的格式是 { success: true, message: aiResponse, sessionId: ... }
        const aiResponse: GeminiChatResult = responseData.message;
