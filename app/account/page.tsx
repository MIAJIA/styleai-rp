    ...
    <div className="container mx-auto p-8 max-w-4xl">
      ...
      <div className="bg-white p-6 rounded-lg shadow-md">
        ...
        <p className="mb-6 text-gray-600">
          Click the button below to fetch the current list of your resource packages and their
          remaining balance from the last 90 days.
        </p>
        <button
        ...
        <div className="mt-6 bg-white p-6 rounded-lg shadow-md">
          ...
          {balanceInfo.data?.resource_pack_subscribe_infos?.length > 0 ? (
            <ul className="space-y-4">
              {balanceInfo.data.resource_pack_subscribe_infos.map(
                (pack: ResourcePack, index: number) => (
                  <li key={index} className="p-4 bg-gray-50 border border-gray-200 rounded-lg">
                    ...
                    <div className="grid grid-cols-2 gap-2 mt-2 text-sm">
                      <p>
                        ...
                        <span
                          className={`px-2 py-1 rounded-full text-xs font-medium ${pack.status === "online" ? "bg-green-100 text-green-800" : "bg-yellow-100 text-yellow-800"}`}
                        >
                          {pack.status}
                        </span>
                      </p>
                      <p>
                        <strong>Total:</strong> {pack.total_quantity}
                      </p>
                      <p>
                        <strong>Remaining:</strong>{" "}
                        <span className="font-bold text-indigo-600">{pack.remaining_quantity}</span>
                      </p>
                      <p></p> {/* Empty cell for alignment */}
                      <p className="col-span-2">
                        <strong>Effective:</strong> {formatDate(pack.effective_time)}
                      </p>
                      <p className="col-span-2">
                        <strong>Expires:</strong> {formatDate(pack.invalid_time)}
                      </p>
                    </div>
                  </li>
                ),
            ...
            <p className="mt-2 text-gray-500">
              No active resource packages found in your account for the last 90 days.
            </p>
          )}
          <div className="mt-6">
