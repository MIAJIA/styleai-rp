...
2. **获取风格化所需数据 (2/7)**
    ...
    - **目的**: `kling-v2`模型需要Base64格式的图片来生成风格化的背景和姿态。
...
帮我设计用户体验 ：目标是在 @chat 生成图像的过程中 请帮我把过程图片在chat里面渲染出来， engage with user： 1`kling-v2`api返回的图像结果； 2 kolors-virtual-try-on api返回的图像结果 最后再返回现在的final image - face swap返回的图像结果；。
根据上面设计工程实现 注意把新的中间图片的显示放在flag最后；这样我可以随时灵活调整这个功能的开关
