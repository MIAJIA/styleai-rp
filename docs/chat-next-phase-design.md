...
**在阶段4验证成功，且多Agent系统稳定运行后**，可以考虑集成图片分析功能，进一步提升专家的专业能力。

### **核心理念：图片分析作为Tool**

- **Function Call架构**：图片分析功能将作为专家Agent可以调用的一个工具（Tool），而不是一个独立的Agent。
- **专家使用工具**：用户将图片发给专家（如小雅），专家在后台调用图片分析工具，然后结合分析结果和自身专业知识给出建议。
- **用户体验自然**：用户感觉是在和专家对话，而不是和一个"图片分析机器人"对话。
...
const analyzeImageTool = {
  ...
  description: "分析用户上传的穿搭照片，提取服装信息和风格特征用于专业建议",
  parameters: { /* ... 省略详细参数 ... */ }
};
```
...
- 扩展现有 `/api/chat/simple` API，增加 `imageUrl` 参数。
- 在调用OpenAI API时，动态加入 `tools` 和 `tool_choice` 参数。
- 增加处理 `tool_calls` 的逻辑，将分析结果返回给Agent进行二次加工。

### **实施步骤**

1. **定义Tool Schema**：明确图片分析工具的输入和输出。
2. **修改API**：支持图片URL和Tool调用。
3. **更新Agent Prompts**：指导Agent在收到图片时如何使用工具。
4. **前端集成**：添加图片上传按钮和分析结果的可选显示。

这个扩展将使我们的多Agent系统更加强大和实用，但应在MVP的核心价值得到充分验证后再进行。
